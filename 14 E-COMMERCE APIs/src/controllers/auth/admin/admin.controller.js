...
module.exports.registerAdmin = async (req, res) => {
  try {
    const admin = await adminAuthService.fetchSingleAdmin(
    ...
    if (admin) {
      return res
        ...
        .json(
          errorResponse(statusCode.BAD_REQUEST, true, MSG.ADMIN_ALREADY_EXISTS),
        );
    }
    ...
    return res
      ...
      .json(
        successResponse(
          statusCode.CREATED,
          false,
          MSG.ADMIN_REGISTRATION_SUCCESS,
          newAdmin,
        ),
      );
  } catch (err) {
  ...
  try {
    ...
    if (!isPassword) {
      return res
        .status(statusCode.BAD_REQUEST)
        .json(
          errorResponse(statusCode.BAD_REQUEST, true, MSG.ADMIN_LOGIN_FAILED),
        );
    }
    ...
    const JWT_SECRET_KEY =
      "heSecretKeyEcommerce0721ForAuthentication&&AuthorizingUsingMySecretKey";
    const token = jwt.sign(payload, JWT_SECRET_KEY, { expiresIn: "7d" });

    console.log(token);

    return res.status(statusCode.OK).json(
      successResponse(statusCode.OK, false, MSG.ADMIN_LOGIN_SUCCESS, {
        token,
      }),
    );
  } catch (err) {
...
module.exports.forgotPassword = async (req, res) => {
  try {
    ...
    await adminAuthService.updateAdmin(admin.id, {
      OTP: OTP,
    ...
    return res
      ...
      .json(successResponse(statusCode.OK, false, MSG.OTP_SEND));
  } catch (err) {
    console.log("Error : ", err);
  }
};
...
module.exports.verifyOTP = async (req, res) => {
  try {
    ...
    if (admin.OTP_Expire < Date.now()) {
      ...
      return res
        .status(statusCode.BAD_REQUEST)
        .json(errorResponse(statusCode.BAD_REQUEST, true, MSG.OTP_EXPIRED));
    }
    ...
    await adminAuthService.updateAdmin(admin.id, {
      ...
      verify_attempt_expire: new Date(Date.now() + 1000 * 60 * 60),
    });
...
module.exports.newPassword = async (req, res) => {
  try {
    ...
    const updatedPassword = await adminAuthService.updateAdmin(admin._id, {
      ...
      return res
        ...
        .json(
          errorResponse(
            statusCode.BAD_REQUEST,
            true,
            MSG.ADMIN_PASSWORD_UPDATE_FAILED,
          ),
        );
    }
    ...
    return res
      .status(statusCode.OK)
      .json(successResponse(statusCode.OK, false, MSG.ADMIN_PASSWORD_UPDATED));
  } catch (err) {
    console.log("Error : ", err);
  }
};
  ...
  try {
    ...
    if (req.user) {
      return res
        .status(statusCode.BAD_REQUEST)
        .json(
          errorResponse(statusCode.BAD_REQUEST, true, MSG.UNAUTHORIZED_ACCESS),
        );
    }
...
module.exports.deleteAdmin = async (req, res) => {
  try {
    ...
    const admin = await adminAuthService.fetchSingleAdmin(
      { _id: req.query.id, isDelete: false, isActive: true },
      true,
    );
    ...
    const deletedAdmin = await adminAuthService.updateAdmin(req.query.id, {
      isDelete: true,
      isActive: false,
    });
...
module.exports.updateAdmin = async (req, res) => {
  try {
    ...
    const admin = await adminAuthService.fetchSingleAdmin(
      { _id: req.params.id, isDelete: false, isActive: true },
      true,
    );
    ...
    const updatedAmdin = await adminAuthService.updateAdmin(
      req.params.id,
      req.body,
    );
...
module.exports.activeOrInActiveAdmin = async (req, res) => {
  try {
    ...
    const updatedAdmin = await adminAuthService.updateAdmin(req.query.id, {
    ...
    return res
      ...
      .json(
        successResponse(
          statusCode.OK,
          false,
          `${admin.first_name} ${admin.last_name} is ${updatedAdmin.isActive ? "active" : "inactive"}`,
        ),
      );
  } catch (err) {
    console.log("Error : ", err);
    ...
    return res
      ...
      .json(
        successResponse(
          statusCode.OK,
          false,
          MSG.ADMIN_PROFILE_FETCH_SUCCESS,
          req.admin,
        ),
      );
  } catch (err) {
...
module.exports.changePassword = async (req, res) => {
  try {
    ...
    await adminAuthService.updateAdmin(req.admin.id, {
      password: req.body.new_password,
    });

    return res
      .status(statusCode.OK)
      .json(successResponse(statusCode.OK, false, MSG.CHANGE_PASSWORD));
  } catch (err) {
    console.log("Error : ", err);
  }
};
